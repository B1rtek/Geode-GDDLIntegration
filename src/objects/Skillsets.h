...
public:
    ...
    Skillset(const std::string &icon, const std::string &name, const std::string &description, float iconScale = 1.0f, IconType iconType = IconType::Cube, int iconNumber = -1) : icon(
            icon), name(name), description(description), iconScale(iconScale), iconType(iconType), iconNumber(iconNumber) {}

    CCNode * getButtonSprite() const {
        if (iconNumber != -1) {
            // weird stuff, thank you dankmeme01 again <3
            const auto simplePlayer = SimplePlayer::create(0);
            if (simplePlayer == nullptr) return nullptr;
            simplePlayer->updatePlayerFrame(iconNumber, iconType);
            // content size fix from dank (expanded upon in a dumb way by me)
            simplePlayer->setContentSize(simplePlayer->m_outlineSprite->getScaledContentSize());
            simplePlayer->m_outlineSprite->setPosition(simplePlayer->m_outlineSprite->getScaledContentSize() / 2);
            simplePlayer->m_firstLayer->setPosition(simplePlayer->m_outlineSprite->getScaledContentSize() / 2);
            simplePlayer->m_secondLayer->setPosition(simplePlayer->m_outlineSprite->getScaledContentSize() / 2);
            simplePlayer->m_detailSprite->setPosition(simplePlayer->m_outlineSprite->getScaledContentSize() / 2);
            simplePlayer->setScale(iconScale * 0.5f);
            return simplePlayer;
        }
    ...
    std::pair<std::string, float> getIconSpriteDetails() const {
        return {icon, iconScale * 0.5f};
    }
...
public:
    ...
    const static inline std::vector<Skillset> skillsetsList = {
            ...
            Skillset("player_105_001.png", "Nerve Control", "This level tests your consistency and ability to handle stress near the end of the level.", 1.0f, IconType::Cube, 105),
            Skillset("player_80_001.png", "Memory", "This level requires remembering a complex path to complete, usually with several fakes, potential routes, and/or visual obscurity.", 1.0f, IconType::Cube, 80),
            Skillset("player_ball_72_001.png", "Learny", "This level needs a significant time investment in order to understand its complex/unintuitive gameplay.", 1.0f, IconType::Ball, 72),
            ...
            Skillset("player_180_001.png", "Chokepoints", "This level contains parts with very condensed difficulty in relation to the rest of the level.", 1.0f, IconType::Cube, 180),
            Skillset("player_113_001.png", "High CPS", "This level has several sections that require very fast (usually controlled) inputs.", 1.0f, IconType::Cube, 113),
            ...
            Skillset("player_ball_106_001.png", "Gimmicky", "This level primarily focuses on developing an experimental, unorthodox gameplay type.", 1.0f, IconType::Ball, 106),
